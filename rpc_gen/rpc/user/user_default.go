...
func GetEmail(ctx context.Context, req *user.DeleteReq, callOptions ...callopt.Option) (resp *user.GetEmailResp, err error) {
	resp, err = defaultClient.GetEmail(ctx, req, callOptions...)
	if err != nil {
		klog.CtxErrorf(ctx, "GetEmail call failed,err =%+v", err)
		return nil, err
	}
	return resp, nil
}
